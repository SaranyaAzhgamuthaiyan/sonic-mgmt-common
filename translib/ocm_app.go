...
func (app *OcmApp) translateSubscribe(dbs [db.MaxDB]*db.DB, path string) (*notificationOpts, *notificationInfo, error) {
	...
	if strings.Contains(path, "/channel-monitor/channels") {
		tblName = "OCM"
		dbKey = asKey("*")
	} else if strings.Contains(path, "/channel/state") {
		tblName = "OCM"
		dbKey = asKey("*")
	} else {
...
func (app *OcmApp) processGetRegex(mdb db.MDB) ([]GetResponseRegex, error) {
	...
	if strings.Contains(app.path.Path, "/channel-monitor/channels") {
		params := &regexPathKeyParams{
			tableName:    "OCM",
			listNodeName: []string{"channel-monitor"},
			keyName:      []string{"name"},
			redisPrefix:  []string{""},
		}
		pathWithKey = constructRegexPathWithKey(mdb, db.StateDB, app.path.Path, params)
	} else if strings.Contains(app.path.Path, "/channel/state") {
		params := &regexPathKeyParamsForMultipleKeyCase{
			tableName:    "OCM",
			listNodeName: []string{"channel-monitor", "channel"},
			keyName:      [][]string{{"name"}, {"lower-frequency", "upper-frequency"}},
			redisPrefix:  [][]string{{""}, {"", ""}},
		}
		pathWithKey = constructRegexPathWithKeyForMultipleKeyCase(mdb, db.StateDB, app.path.Path, params)
	} else {
