...
func Test_node_on_openconfig_sflow(t *testing.T) {
	...
	time.Sleep(1 * time.Second)
	t.Run("Verify set on sflow node", verifyDbResult(rclient[hostDBName], "SFLOW|global", expected_map, false))
	time.Sleep(1 * time.Second)
	...
	time.Sleep(1 * time.Second)
	t.Run("Verify polling-interval on sflow node", verifyDbResult(rclient[hostDBName], "SFLOW|global", expected_map, false))
	time.Sleep(1 * time.Second)
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	time.Sleep(1 * time.Second)
	...
	time.Sleep(1 * time.Second)
	t.Run("Verify Agent on sflow node", verifyDbResult(rclient[hostDBName], "SFLOW|global", expected_map, false))
	time.Sleep(1 * time.Second)
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	time.Sleep(1 * time.Second)
...
func Test_node_openconfig_sflow_collector(t *testing.T) {
	...
	time.Sleep(1 * time.Second)
	t.Run("Verify set on collector node for sflow", verifyDbResult(rclient[hostDBName], "SFLOW_COLLECTOR|1.1.1.1_6343_default", expected_map, false))
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	time.Sleep(1 * time.Second)
	...
	t.Run("Verify delete on collector node for sflow", verifyDbResult(rclient[hostDBName], "SFLOW_COLLECTOR|2.2.2.2_4444_default", delete_expected, false))
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	time.Sleep(1 * time.Second)
...
func Test_node_openconfig_sflow_interface(t *testing.T) {
	...
	t.Run("Verify set on sflow interface", verifyDbResult(rclient[hostDBName], "SFLOW_SESSION|Ethernet0", expected_map, false))
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	time.Sleep(2 * time.Second)
	...
	loadDB(hostDBName, db.ConfigDB, pre_req_map)
	loadDB(hostDBName, db.ApplDB, non_pre_req_map)
	time.Sleep(2 * time.Second)
	...
	time.Sleep(2 * time.Second)
	delete_expected := make(map[string]interface{})
	t.Run("Verify delete on sflow interface", verifyDbResult(rclient[hostDBName], "SFLOW_SESSION|Ethernet4", delete_expected, false))
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	unloadDB(hostDBName, db.ApplDB, non_pre_req_map)
	time.Sleep(2 * time.Second)
	...
	time.Sleep(2 * time.Second)
	t.Run("Verify admin-state for sflow interface", verifyDbResult(rclient[hostDBName], "SFLOW_SESSION|Ethernet0", expected_map, false))
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	unloadDB(hostDBName, db.ApplDB, non_pre_req_map)
	time.Sleep(2 * time.Second)
	...
	time.Sleep(2 * time.Second)
	t.Run("Verify sampling-rate on sflow interface", verifyDbResult(rclient[hostDBName], "SFLOW_SESSION|Ethernet0", expected_map, false))
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl)
	unloadDB(hostDBName, db.ConfigDB, cleanuptbl_sflow)
	unloadDB(hostDBName, db.ApplDB, non_pre_req_map)
	time.Sleep(2 * time.Second)
